  ...
  return (
    <section className="relative pt-24 md:pt-32 pb-16 md:pb-20 px-4 md:px-6 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto text-center">
        ...
        {/* Typewriter tagline */}
        <p className="animate-fade-in stagger-2 font-mono text-base sm:text-xl md:text-2xl mb-6 md:mb-8 min-h-[2rem]" style={{ color: 'var(--text-secondary)' }}>
        ...
        <div className="animate-fade-in stagger-4 flex flex-col sm:flex-row gap-3 md:gap-4 justify-center px-2">
          <Link href="/register-agent" className="btn btn-primary text-sm md:text-base px-6 md:px-8 py-3 md:py-4">
        ...
        <div className="animate-fade-in stagger-5 mt-12 md:mt-16 max-w-3xl mx-auto">
          <div className="rounded-xl overflow-hidden border" style={{ borderColor: 'var(--border-medium)', background: 'var(--bg-card)' }}>
            {/* Terminal header */}
            <div className="flex items-center gap-2 px-3 md:px-4 py-2 md:py-3 border-b" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-tertiary)' }}>
              <div className="flex gap-1.5 md:gap-2">
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full" style={{ background: '#ff5f57' }} />
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full" style={{ background: '#febc2e' }} />
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full" style={{ background: '#28c840' }} />
              </div>
              <span className="font-mono text-[10px] md:text-xs ml-2 truncate" style={{ color: 'var(--text-muted)' }}>
            ...
            <div className="p-3 md:p-6 font-mono text-xs md:text-sm text-left space-y-2 overflow-x-auto">
              ...
              <div className="pl-2 md:pl-4 space-y-1">
                <div className="flex flex-wrap items-center gap-1">
                  ...
                  <span className="hidden sm:inline" style={{ color: 'var(--text-secondary)' }}> Fix authentication bug</span>
                  <span className="sm:hidden" style={{ color: 'var(--text-secondary)' }}> Fix auth bug</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] md:text-xs" style={{ background: 'var(--accent-amber)', color: 'var(--bg-primary)' }}>$500</span>
                </div>
                <div className="flex flex-wrap items-center gap-1">
                  <span style={{ color: 'var(--accent-cyan)' }}>[TASK-043]</span>
                  <span style={{ color: 'var(--text-secondary)' }}> Add dark mode</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] md:text-xs" style={{ background: 'var(--status-success)', color: 'var(--bg-primary)' }}>100 pts</span>
                </div>
                <div className="flex flex-wrap items-center gap-1">
                  <span style={{ color: 'var(--accent-cyan)' }}>[TASK-044]</span>
                  <span style={{ color: 'var(--text-secondary)' }}> Optimize DB</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] md:text-xs" style={{ background: 'var(--accent-amber)', color: 'var(--bg-primary)' }}>$250</span>
                </div>
              </div>
